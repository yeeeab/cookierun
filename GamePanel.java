...
public class GamePanel extends JPanel {
    ...
    Color backFade; // 배경이 바뀔 때 페이드아웃 페이드 인 하기 위한 컬러변수
    ...
    public GamePanel(JFrame superFrame, CardLayout cl, Object o) {
        ...
        // 코인 두배 포션 초기화
        coinPotion = new CoinPotion();

        // Load the speeduppotion image
        ImageIcon potionIcon3 = new ImageIcon("img/store/potion3.png");
        Image potionImage3 = potionIcon3.getImage();
        Image resizedPotionImage3 = potionImage3.getScaledInstance(60, 80, Image.SCALE_SMOOTH); // Resize to 60x80
        ImageIcon resizedPotionIcon3 = new ImageIcon(resizedPotionImage3);

		// Load the healthpotion image
        ImageIcon potionIcon2 = new ImageIcon("img/store/potion2.png");
        Image potionImage2 = potionIcon2.getImage();
        Image resizedPotionImage2 = potionImage2.getScaledInstance(60, 80, Image.SCALE_SMOOTH); // Resize to 60x80
        ImageIcon resizedPotionIcon2 = new ImageIcon(resizedPotionImage2);

        // Initialize coinPotion
        // coinPotion = new CoinPotion(resizedPotionImage1, 40, 0, resizedPotionIcon1.getIconWidth(), resizedPotionIcon1.getIconHeight(), 1);

		// Load the coinpotion image
        ImageIcon potionIcon1 = new ImageIcon("img/store/potion1.png");
        Image potionImage1 = potionIcon1.getImage();
        Image resizedPotionImage1 = potionImage1.getScaledInstance(60, 80, Image.SCALE_SMOOTH); // Resize to 60x80
        ImageIcon resizedPotionIcon1 = new ImageIcon(resizedPotionImage1);

        // Initialize coinPotionBtn
        coinPotionBtn = new JButton(resizedPotionIcon1);
        coinPotionBtn.setName("coinPotionBtn");
        int buttonWidth1 = resizedPotionIcon1.getIconWidth();
        int buttonHeight1 = resizedPotionIcon1.getIconHeight();
        coinPotionBtn.setBounds((superFrame.getWidth() - buttonWidth1) / 2 + 100, superFrame.getHeight() - buttonHeight1 - 40, buttonWidth1, buttonHeight1); // Position the button at the bottom right
        coinPotionBtn.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                if (!coinPotion.isUsed()) {
                    coinPotion.isUsed();
                }
            }
        });
		...
		// Initialize healthPotionBtn
        healthPotionBtn = new JButton(resizedPotionIcon2);
        healthPotionBtn.setName("healthPotionBtn");
        int buttonWidth2 = resizedPotionIcon2.getIconWidth();
        int buttonHeight2 = resizedPotionIcon2.getIconHeight();
        healthPotionBtn.setBounds((superFrame.getWidth() - buttonWidth2) / 2 + 100, superFrame.getHeight() - buttonHeight2 - 40, buttonWidth2, buttonHeight2); // Position the button at the bottom right
        healthPotionBtn.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
        ...
        this.add(healthPotionBtn);

		// Initialize speedUpPotionBtn
        speedUpPotionBtn = new JButton(resizedPotionIcon3);
        speedUpPotionBtn.setName("speedUpPotionBtn");
        int buttonWidth3 = resizedPotionIcon3.getIconWidth();
        int buttonHeight3 = resizedPotionIcon3.getIconHeight();
        speedUpPotionBtn.setBounds((superFrame.getWidth() - buttonWidth3) / 2 + 100, superFrame.getHeight() - buttonHeight3 - 40, buttonWidth3, buttonHeight3); // Position the button at the bottom right
        speedUpPotionBtn.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                if (!speedUpPotion.isUsed()) {
                    speedUpPotion.isUsed();
                }
            }
        });
        this.add(speedUpPotionBtn);
    }
    private int healAmount = 0; // 기본값을 설정하거나 초기화 필요
    ...
    // 체력증가 포션 체력 증가
    public void increaseHealthTemporarily(int amount, int durationMillis) {
        new Thread(() -> {
            this.healAmount += amount;

            try {
                Thread.sleep(durationMillis); // 지정된 기간 동안 일시 중지
    ...
    // 코인증가 포션 코인 두배
    public void doubleCoinsTemporarily(int durationMillis) {
        new Thread(() -> {
            int originalCoinScore = this.coinScore;
            this.coinScore *= 2;
            ...
            this.coinScore = originalCoinScore;
        }).start();
    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                while (true) {
                    ...
                    if (runPage > 800) { // 800픽셀 이동 마다 체력이 10씩 감소한다 (추후 맵길이에 따라 감소량 조절)
                    ...
                    for (int i = 0; i < coinList.size(); i++) {
                        ...
                        if (tempCoin.getX() < -90) {
                            ...
                            if (c1.getImage() != slideIc.getImage()
                                ...
                                coinScore = coinScore + (coinPotion.isUsed() ? tempCoin.getScore() * 2 : tempCoin.getScore());
                                coinList.remove(tempCoin);
                            } else if (c1.getImage() == slideIc.getImage()
                                    && tempCoin.getX() + tempCoin.getWidth() * 20 / 100 >= c1.getX()
                                    && tempCoin.getX() + tempCoin.getWidth() * 80 / 100 <= face
...
private void jump() {
    new Thread(new Runnable() {
        ...
        public void run() {
            ...
            int jumpY = 1; // 1 이상으로만 설정하면 된다.(while문이 처음에 작동하기 위해)

            while (jumpY > 0) {
                t2 = Util.getTime() - t1; // 지금 시간에서 t1을 뺀다
                jumpY = set - (int) ((t2) / 30); // 위로 올라갈수록 속도가 줄어든다.

                c1.setY(c1.getY() - jumpY); // Y좌표에 점프량을 뺀다

                if (nowJump != c1.getCountJump()) { // 점프가 도중에 더블 점프로 변경되면 끝낸다
                    break;
                }

                if (escKeyOn) { // esc 키를 누르면 게임이 멈춘다
                    while (escKeyOn) {
                        try {
                            Thread.sleep(10);
                        } catch (InterruptedException e) {
            ...
            c1.setJump(false); // 점프가 끝났기 때문에 점프 중을 false로 변경
        }
    }).start();
}
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                while (alpha < 255) {
                    ...
                    backFade = new Color(0, 0, 0, alpha);
